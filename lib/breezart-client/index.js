...
class BreezartClient {
  constructor(options) {
    ...
    this.connection.on('connect', () => {
      ...
      this.commands.start((err, results) => {
        if (err) {
          // throw err;
      ...
      this.send(req, (error, response) => {
        ...
        if (error) {
          ...
          if (!response) {
            ...
            let parcedResponse = response
              .split(BreezartClient.DELIMITER)
              .filter((v) => {
                return !!v;
              });
  ...
  connect(connectionParams, attempts) {
    ...
    this.connection.once('error', () => {
      ...
      setTimeout(
        (t, a) => {
          console.error(
            'Connection failed: Reconnecting after',
            (t / 1000) * 2,
            'secons'
          );
  ...
  getProperties(callback) {
    ...
    this.makeRequest(requestType, null, (error, response) => {
      if (error) {
        ...
        callback(
          new Error(`Cant get properties of Breezart: ${error.message}`)
        );
      } else {
        ...
        if (response[0] !== requestType) {
          callback(
            // prettier-ignore
            new Error(`Incorrect response received from Breezart. Must be: ${requestType}, but received: ${response[0]}`)
          );
          return;
        }
        // bitTempr
        ...
        console.log('Get properties status: successfully');
        callback();
      }
    });
  }
  ...
  */
  getStatus(callback) {
    // get status only after properties are initialized
    this.getProperties(() => {
      let requestType = BreezartClient.RequestPrefix.STATE;
      this.makeRequest(requestType, null, (error, response) => {
        if (error) {
          // Properties need for next work
          callback(new Error(`Can't get status of Breezart: ${error.message}`));
          return;
        } else {
          ...
          if (response[0] !== requestType) {
            callback(
              // prettier-ignore
              new Error(`Incorrect response received form Breezart. Must be ${requestType}, but received: ${response[0]}`)
            );
            return;
          }
          // bitState
  ...
  getSersorValues(callback) {
    ...
    this.makeRequest(requestType, null, (error, response) => {
      if (error) {
        // Properties need for next work
        callback(
          new Error(`Can't get sensor values of Breezart: ${error.message}`)
        );
      } else {
        // Response is array from `VSens_TInf_HInf_TRoom_HRoom_TOut_HOut_THF_Pwr`
        if (response[0] !== requestType) {
          callback(
            new Error(
              `Incorrect response received form Breezart. Must be ${requestType}, but received: ${response[0]}`
            )
          );
          return;
        }
        ...
        this.TInf =
          response[1] === 'fb07' ? null : hexToDecSign(response[1]) / 10.0;
        this.HInf =
          response[2] === 'fb07' ? null : hexToDec(response[2]) / 10.0;
  ...
  setPower(power, callback) {
    ...
    this.getStatus(() => {
      ...
      let data = power
        ? BreezartClient.DataValues.POWER_ON
        : BreezartClient.DataValues.POWER_OFF;
      this.makeRequest(requestType, data, (error, response) => {
        if (error) {
          // Properties need for next work
          callback(
            new Error(`Can't get sensor values of Breezart: ${error.message}`)
          ...
          if (response[0] !== 'OK' && response[1] !== requestType) {
            callback(
              // prettier-ignore
              new Error(`Incorrect response received form Breezart. Must be OK, but received: ${response[0]}`)
            );
            return;
          }
          callback();
  ...
  setRotationSpeed(value, callback) {
    this.getStatus(() => {
      // set rotation speed only when power on
      if (this.UnitState === 0 || this.UnitState === 2) {
        callback();
        return;
      }

      if (value === this.Speed || value === this.SpeedTarget) {
        callback();
        return;
      }

      const requestType = BreezartClient.RequestPrefix.CHANGE_FAN_SPEED;
      this.makeRequest(requestType, value, (error, response) => {
        if (error) {
          // Properties need for next work
          callback(
            new Error(`Can't get sensor values of Breezart: ${error.message}`)
